...
function addToDOM (application) {
  fetch(
    '/projectInvitation/' + application.project_id + '/' + application.sender + '/' + application.receiver
  ).then(res =>
    res.json().then(data => {
      invitation_list.innerHTML = invitation_list.innerHTML +
        `
        <br> 
...
fetch('/allRequests').then(res => {
  res.json().then(data => {
    ...
    if (data.length == 0) {
      invitation_list.innerHTML = "<p class='h3 text-primary'>No invitations yet.</p>"
    }
  })
})
