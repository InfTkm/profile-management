...
function quit_project_dom(project_id){
    ...
    if (document.querySelector('#content').children.length === 0){
        ...
        placeholder.innerHTML = 'It looks like you don\'t have any projects right now.'
...
function finish_project(id){
    const project = document.querySelector('#' + id).querySelector('#teammateContainer').querySelector('ul')
    if (project.children.length === 1){
        alert('Please quit the project instead.')
    }
    else{
